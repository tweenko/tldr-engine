if !visible exit
if sprite_index==spr_block exit

//(0.8+global.platforming_perspective*0.2)
//clamp(global.platforming_perspective*4, 0, 1)

// Stuff
if global.platforming_perspective==1 {floorbackclamping=linear(floorbackclamping, 2, 0.5)} else {floorbackclamping=linear(floorbackclamping, lerp(floor_tilesback_amount, 2, clamp(-0.7+global.platforming_perspective, 0, 1)), 1)}
...
draw_sprite_ext(sprite_index, subimg_auto_sprite(sprite_index), 0, 0, image_xscale, image_yscale, 0, blend, image_alpha*plathidden)
...
if floor_tilesback_amount>0{
	...
	draw_sprite_stretched_ext(floor_sprite, subimg_auto_sprite(floor_sprite), 0, -floor_height, floor_width, floor_height, blend, image_alpha*plathidden)
}
