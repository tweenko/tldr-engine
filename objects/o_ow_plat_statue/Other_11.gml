...
if global.platforming_perspective == 0 {
	...
	cutscene_animate(0, 1, 14, "quad_in_out", global, "platforming_perspective");
    
	for (var i = 0; i < party_length(true); ++i) {
		var inst = party_get_inst(global.party_names[i]);
        var offset_x = ((i + 1) div 2) * cos(i * pi) * 20;
		cutscene_animate(inst.x, x + offset_x, 14, "quad_in_out", inst, "x");
		...
		if instance_exists(plat_parent) 
            cutscene_animate(inst.y, plat_parent.initial_y - (-(plat_parent.tile_height * plat_parent.wall_distance)), 14, "quad_in_out", inst, "y");
	...
	cutscene_animate(1, 0, transtime_2, "quad_in_out", global, "platforming_perspective");
    
	for (var i = 0; i < party_length(true); ++i) {
		var inst = party_get_inst(global.party_names[i]);
        var offset_x = ((i + 1) div 2) * cos(i * pi) * 20;
        
		cutscene_animate(inst.x, x + offset_x, transtime_2, "quad_in_out", inst, "x");
		cutscene_animate(inst.y, initial_y + 15, transtime_2, "quad_in_out", inst, "y");
	}
