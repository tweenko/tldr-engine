...
if state == -1 {
	...
	if InputPressed(INPUT_VERB.SELECT) {
		...
		if sselection == 0 {
			o_world.chapters[tselec-1].exec(id) // run the chapter start script
			lock = true
		}
		else {
			yadd = -80
	...
	if InputPressed(INPUT_VERB.DOWN) && !confirming {
		if selection >= total + 1 {
			...
			audio_play(snd_ui_move)
			while selection < total + 1 && !is_struct(o_world.chapters[selection-1]) 
				selection++
		}
	}
		...
		if selection > 1 {
			...
			while selection > 0 && !is_struct(o_world.chapters[selection-1]) 
				selection--
			if selection==0 
				selection=_save
		}
	...
	if InputPressed(INPUT_VERB.SELECT) {
		if selection <= total {
			if is_struct(o_world.chapters[selection - 1]) {
				if confirming == true && confirmselection == 0{
					audio_play(snd_ui_select)
					o_world.chapters[selection - 1].exec(id)
					lock = true
				}
