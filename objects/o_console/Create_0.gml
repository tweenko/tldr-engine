...
for (var i = 0; i < array_length(registred_commands); i ++) {
	var command = {};
	with (command) script_execute(other.registred_commands[i]); // call the constructor from the script
	if (!is_array(command.hotkey)) command.hotkey = [command.hotkey];
	registred_commands[i] = command;
}
