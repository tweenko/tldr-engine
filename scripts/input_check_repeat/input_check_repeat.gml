...
function input_check_repeat(_verb, _player_index = 0, _delay = INPUT_REPEAT_DEFAULT_DELAY, _predelay = INPUT_REPEAT_DEFAULT_PREDELAY)
{
    ...
    if (_global.__cleared || _verb_struct.__inactive || !_verb_struct.held) return false;
...
/// @desc    Returns if the verb has been re-triggered by a repeater behaviour or just simply pressed.

/// @param   verb
/// @param   playerIndex=0
/// @param   delay
/// @param   predelay
function input_check_pressed_repeat(_verb, _player_index=0, _delay=INPUT_REPEAT_DEFAULT_DELAY, _predelay=INPUT_REPEAT_DEFAULT_PREDELAY){
	return input_check_pressed(_verb)||input_check_repeat(_verb,_player_index,_delay,_predelay)
}